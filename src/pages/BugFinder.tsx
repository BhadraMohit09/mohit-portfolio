...
export function BugFinder({ onBack }: { onBack: () => void }) {
  ...
  const snippet = SNIPPETS[level];
  const lines = snippet.code.split('\n');
  const isGameOver = level >= SNIPPETS.length;
      ...
      <Shell className="px-4 sm:px-8 flex-grow flex flex-col">
        ...
        <div className="flex items-center justify-between mb-8 sm:mb-12">
          ...
          <div className="flex items-center gap-6 font-mono text-[12px]">
            <span className="text-[var(--fg)] font-bold">Level: {isGameOver ? SNIPPETS.length : level + 1} / {SNIPPETS.length}</span>
          ...
          {!isGameOver ? (
            <>
              <div className="text-center mb-6">
                ...
                <p className="text-[var(--muted)] text-sm">Click the line of code that contains the error.</p>
              </div>

              <div className="w-full bg-[#0d1117] border border-[var(--line)] rounded-xl overflow-hidden font-mono text-sm shadow-xl">
                <div className="flex items-center px-4 py-2 bg-[#161b22] border-b border-[var(--line)] text-xs text-[#8b949e]">
              ...
              <AnimatePresence>
                {showResult && (
                  ...
                  >
                    ...
                    <button 
                      onClick={nextLevel}
                      className="px-6 py-2 bg-[var(--fg)] text-[var(--bg)] font-mono text-xs font-bold rounded-lg transition-transform hover:scale-105"
                    >
                      Next Snippet ➔
                    </button>
                  </motion.div>
            ...
            <div className="text-center mt-12">
              <h2 className="text-3xl font-serif mb-4 text-[var(--fg)]">Code Review Complete!</h2>
              <p className="text-[var(--muted)] mb-8">You found {score} out of {SNIPPETS.length} bugs.</p>
              <button 
                onClick={reset}
                className="px-8 py-3 bg-[var(--fg)] text-[var(--bg)] font-mono text-sm font-bold rounded-xl transition-transform hover:scale-105"
              >
                Play Again
              </button>
            </div>
